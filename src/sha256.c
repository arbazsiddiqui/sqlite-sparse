...
static int sha256_file_hex(const char *path, char out[65]) {
    ...
    fclose(f); sha256_hex(&s, out); return 0;
}

static void sha256_bytes_hex(const void *data, size_t len, char out[65]) {
    Sha256 s; sha256_init(&s); sha256_update(&s, data, len); sha256_hex(&s, out);
}
